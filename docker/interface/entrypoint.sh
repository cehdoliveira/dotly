...
if [ -f "/var/www/infinnityimportacao/site/app/inc/lib/composer.json" ]; then
    echo "Instalando dependências do site..."
    cd /var/www/infinnityimportacao/site/app/inc/lib
    composer install --no-interaction --prefer-dist --optimize-autoloader
fi

if [ -f "/var/www/infinnityimportacao/manager/app/inc/lib/composer.json" ]; then
    echo "Instalando dependências do manager..."
    cd /var/www/infinnityimportacao/manager/app/inc/lib
    composer install --no-interaction --prefer-dist --optimize-autoloader
fi
...
SITE_HTTP_HOST=$(php -r '$_SERVER["HTTP_HOST"]=""; require "/var/www/infinnityimportacao/site/app/inc/kernel.php"; $h=explode(",", constant("ALLOWED_HOSTS")); echo trim($h[0]);' 2>/dev/null || true)
MANAGER_HTTP_HOST=$(php -r '$_SERVER["HTTP_HOST"]=""; require "/var/www/infinnityimportacao/manager/app/inc/kernel.php"; $h=explode(",", constant("ALLOWED_HOSTS")); echo trim($h[0]);' 2>/dev/null || true)
...
supervise_worker /var/www/infinnityimportacao/manager/cgi-bin/kafka_email_worker.php /var/log/kafka_email_worker_manager.log "$MANAGER_HTTP_HOST" &
supervise_worker /var/www/infinnityimportacao/site/cgi-bin/kafka_email_worker.php /var/log/kafka_email_worker_site.log "$SITE_HTTP_HOST" &
