...
sed -i \
    -e "s#define(\"mail_from_name\", \"infinnityimportacao\");#define(\"mail_from_name\", \"${E_BRAND_NAME}\");#" \
    -e "s#define(\"cAppKey\", \"infinnityimportacao_site_session\");#define(\"cAppKey\", \"${E_SLUG}_site_session\");#" \
    -e "s#define(\"cTitle\", \"infinnityimportacao\");#define(\"cTitle\", \"${E_BRAND_NAME}\");#" \
    -e "s#define(\"ALLOWED_HOSTS\", \"infinnityimportacao.local\");#define(\"ALLOWED_HOSTS\", \"${E_SITE_HOSTS}\");#" \
    -e "s#define(\"SITE_CANONICAL_URL\", \"http://infinnityimportacao.local\");#define(\"SITE_CANONICAL_URL\", \"${E_SITE_URL}\");#" \
    -e "s#define(\"REDIS_PREFIX\", \"infinnityimportacao:site:\");#define(\"REDIS_PREFIX\", \"${E_SLUG}:site:\");#" \
    -e "s#define(\"KAFKA_TOPIC_EMAIL\", \"infinnityimportacao_site_emails\");#define(\"KAFKA_TOPIC_EMAIL\", \"${E_SLUG}_site_emails\");#" \
    -e "s#define(\"KAFKA_CONSUMER_GROUP\", \"infinnityimportacao-site-email-worker-group\");#define(\"KAFKA_CONSUMER_GROUP\", \"${E_SLUG}-site-email-worker-group\");#" \
    "$SITE_KERNEL"

cp "$MANAGER_EXAMPLE" "$MANAGER_KERNEL"
sed -i \
    -e "s#define(\"mail_from_name\", \"infinnityimportacao Manager\");#define(\"mail_from_name\", \"${E_BRAND_NAME} Manager\");#" \
    -e "s#define(\"cAppKey\", \"infinnityimportacao_manager_session\");#define(\"cAppKey\", \"${E_SLUG}_manager_session\");#" \
    -e "s#define(\"cTitle\", \"infinnityimportacao Manager\");#define(\"cTitle\", \"${E_BRAND_NAME} Manager\");#" \
    -e "s#define(\"ALLOWED_HOSTS\", \"manager.infinnityimportacao.local\");#define(\"ALLOWED_HOSTS\", \"${E_MANAGER_HOSTS}\");#" \
    -e "s#define(\"MANAGER_CANONICAL_URL\", \"http://manager.infinnityimportacao.local\");#define(\"MANAGER_CANONICAL_URL\", \"${E_MANAGER_URL}\");#" \
    -e "s#define(\"REDIS_PREFIX\", \"infinnityimportacao:manager:\");#define(\"REDIS_PREFIX\", \"${E_SLUG}:manager:\");#" \
    -e "s#define(\"KAFKA_TOPIC_EMAIL\", \"infinnityimportacao_manager_emails\");#define(\"KAFKA_TOPIC_EMAIL\", \"${E_SLUG}_manager_emails\");#" \
    -e "s#define(\"KAFKA_CONSUMER_GROUP\", \"infinnityimportacao-manager-email-worker-group\");#define(\"KAFKA_CONSUMER_GROUP\", \"${E_SLUG}-manager-email-worker-group\");#" \
    "$MANAGER_KERNEL"
