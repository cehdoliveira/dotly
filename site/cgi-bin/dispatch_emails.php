...
// ATENCAO: CLI_HTTP_HOST e obrigatorio em producao (entrypoint.sh deriva de
// ALLOWED_HOSTS[0] e injeta no cron). Fallback vazio so e seguro para
// debugging manual — sem host, links de e-mail serao gerados com "localhost".
...
$got = $rawPdo->query("SELECT GET_LOCK('app_dispatch_emails', 0) AS l")->fetch(\PDO::FETCH_ASSOC);
if ((int)($got['l'] ?? 0) !== 1) {
...
try {
    ...
    $rawPdo->query("SELECT RELEASE_LOCK('app_dispatch_emails')");
}
