...
final class CommonFunctionsTest extends TestCase
{
    ...
    {
        // Suporte a multi-selecao (ex.: filtro de status em /pedidos no manager,
        // mesmo CommonFunctions.php byte-identico entre site/ e manager/):
    ...
    public function test_canonical_url_uses_configured_constant(): void
    {
        ...
        if (!defined('TEST_CANONICAL_URL')) {
            define('TEST_CANONICAL_URL', 'http://infinnityimportacao.local');
        }
        $this->assertSame('http://infinnityimportacao.local', canonical_url('TEST_CANONICAL_URL'));
    }
     ...
     * (ver plano 006 — bug real: GET /produto/{slug}?format=json 500'ava pra
     * qualquer produto sem description/dosage/purity_label preenchidos).
     */
    ...
    /**
     * Regressao: check_and_increment_rate_limit() chamava $redis->incr(), metodo
     * que nunca existiu em RedisCache (so increment()) — nunca pegou porque
     * $GLOBALS['redis'] sempre foi null em producao (bug de ordem de include em
     * main.php, corrigido junto). Este stub simula um RedisCache real/conectado
     * (objeto truthy) sem precisar de conexao de verdade, pra travar os nomes de
     * metodo certos: setIfNotExists() atomico na primeira chamada (grava count=1
     * + TTL numa unica operacao — sem a janela de crash de um incr()+expire() em
     * duas chamadas separadas), increment() nas chamadas seguintes.
     */
    public function testRateLimitWithConnectedRedisUsesAtomicSetThenIncrement(): void
    {
        $redis = new class {
            public array $store = [];
            public array $ttls = [];
            public int $setIfNotExistsCalls = 0;
        ...
        $blocked1 = check_and_increment_rate_limit($redis, 'test:key', 2, 60);
        $blocked2 = check_and_increment_rate_limit($redis, 'test:key', 2, 60);
        $blocked3 = check_and_increment_rate_limit($redis, 'test:key', 2, 60);

        $this->assertFalse($blocked1, 'primeira chamada (count=1) nao deveria bloquear');
        $this->assertFalse($blocked2, 'segunda chamada (count=2, no limite) nao deveria bloquear');
        $this->assertTrue($blocked3, 'terceira chamada (count=3, acima do limite) deveria bloquear');
        $this->assertSame(60, $redis->ttls['test:key'], 'TTL deveria ter sido gravado atomicamente na criacao da chave');
        $this->assertSame(3, $redis->setIfNotExistsCalls, 'setIfNotExists() e chamado toda vez, mas so cria a chave na 1a (as outras 2 retornam false)');
        $this->assertSame(2, $redis->incrementCalls, 'increment() so deveria rodar quando a chave ja existia (2a e 3a chamada)');
    }

    public function testResetRateLimitWithConnectedRedisCallsDelete(): void
    {
        $redis = new class {
        ...
        reset_rate_limit($redis, 'login_attempts:1.2.3.4');

        $this->assertSame(
            ['login_attempts:1.2.3.4'],
            $redis->deleted,
            'reset_rate_limit deveria chamar delete() (metodo real do RedisCache), nao del()'
        );
    }
